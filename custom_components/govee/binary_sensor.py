...
Provides:
- Per-device transport connectivity diagnostics (Cloud API / MQTT / BLE),
  opt-in via the ``expose_transport_entities`` option.
- Leak sensor binary sensors (H5058): moisture detection, sensor/gateway
  connectivity via BFF API polling.
"""
...
async def async_setup_entry(
    ...
    # Transport connectivity diagnostics (opt-in)
    ...
    if entities:
        ...
        _LOGGER.debug("Set up %d Govee binary sensor entities", len(entities))
...
class GoveeTransportConnectivity(GoveeEntity, BinarySensorEntity):
    ...
    def available(self) -> bool:
        ...
        main device's online flag ΓÇö otherwise an offline device would
